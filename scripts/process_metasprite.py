...
"""
  ...
  with open(fout / "header" / "speed_table.bin", 'wb') as outbin,open(fout / "header" / "speed_table.hpp", 'w') as outhpp:
    all_data = []
    for i in [0.4, 0.75, 1.0, 1.1, 1.2, 1.4, 1.65, 1.80]:
      all_data += [float2fixedangles(i)]
    split = tuple(map(list, zip(*all_data)))
    speedname, *fields = split
    speednameout = ",\n\t".join([f"s{x:.2f}".replace(".","_") for x in speedname])
    speednameout_times2 = ",\n\t".join([f"s{x * 2:.2f}".replace(".","_") for x in speedname])
    outhpp.write(f"""
...
def rotate_origin_only(xy, radians):
  ...
  yy = abs(-x * math.sin(radians) + y * math.cos(radians))
  return xx, yy

def float2fixedangles(mag):
  all_angles = []
  count = 32
  for i in range(count):
    x,y = rotate_origin_only(mag, math.radians(i * (360.0/count)))
    print(f"mag: {mag:.2f} x,y {x:.2f},{y:.2f}")
    x = int(x * float(1 << 8))
    y = int(y * float(1 << 8))
    all_angles += [
      x & 0xff,
