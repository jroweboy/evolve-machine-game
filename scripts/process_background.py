...
def main(nestiler: Path, fin: Path, fout: Path):
  ...
  for chr in rawchr_path.glob("*.chr"):
    with open(chr, 'rb') as f:
      raw_chr = f.read()
      l = len(raw_chr)
      chr_offset[chr.stem] = l
      chr_count[chr.stem] = l // 16
...
"""
  for name,off in chr_offset.items():
    asm_file += f"""
.global {name}_chr_offset
{name}_chr_offset = {off}
"""
  asm_file += """
...
"""
  for name,count in chr_count.items():
    asm_file += f"""
.global {name}_chr_count
{name}_chr_count = {count}
"""

  with open(asm_path / "graphics_constants.s", 'w') as out:
    out.write(asm_file)
