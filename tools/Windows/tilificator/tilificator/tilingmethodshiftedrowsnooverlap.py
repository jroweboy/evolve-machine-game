...
class TilingMethodShiftedRowsNoOverlap(TilingMethod):
    def __init__(self, si, tileTable, settings, optimizationSettings):
        super().__init__(si, tileTable, settings, optimizationSettings)
    ...
    def getShiftedRowsTilification(self, lineBegin, lineEnd, shiftY):
        ...
        for ty in range(maxTilesH):
            ...
            numTilesPrev = self.tileTable.numTiles
            pcc = [] #self.perColumnCoverage(sy, min(sy + self.settings.tileHeight, self.si.height))
            xcoordCombinations = generateTileXCoordsNoOverlapSimplified(tileEnd + 1 - tileBegin, self.settings.tileWidth, tileBegin, pcc)
    ...
    def perColumnCoverage(self, yStart, yEnd):
        pcc = [False] * self.si.width
        self.settings.colorSize
        for x in range(self.si.width):
            needsCoverage = any((self.si.data[x, y] % self.settings.colorSize != 0) for y in range(yStart, yEnd))
            pcc[x] = needsCoverage
        return pcc
...
def numContiguous(tileWidth, pcc, comp = True):
    numContiguous = 0
    x = 0
    while x < len(pcc) and pcc[x] == comp:
        x += tileWidth
        numContiguous += 1
    return numContiguous

def generateTileXCoordsNoOverlap(width, tileWidth, xOffs, pcc):
    grpNumTiles = []
    grpGapSize = []
    # Find group size and gap between groups
    offs = xOffs
    while len(pcc[offs:]) > 0:
        numTiles = max(numContiguous(tileWidth, pcc[offs:]), 1)
        gapSize = numContiguous(1, pcc[(offs+numTiles*tileWidth):], False)
        grpNumTiles.append(numTiles)
        grpGapSize.append(gapSize)
        offs += numTiles*tileWidth + gapSize
    xcoords = [list() for i in range(len(grpNumTiles))]
    for i, gapSize in enumerate(grpGapSize):
        for xShift in range(gapSize+1):
            xcoords[i].append(tuple([tileWidth*x + xShift for x in range(grpNumTiles[i])]))
    return list(itertools.product(*xcoords))[0]

def generateTileXCoordsNoOverlapSimplified(width, tileWidth, xOffs, pcc):
    '''
    Super-simple version with no product - only creates one contiguous row.
    '''
    gridWidth = (width + tileWidth-1) // tileWidth
    xcoords = [tileWidth*x + xOffs for x in range(gridWidth)]
    return [xcoords]
