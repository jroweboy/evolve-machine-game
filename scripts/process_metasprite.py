...
"""
  ...
  with open(fout / "header" / "speed_table.bin", 'wb') as outbin,open(fout / "header" / "speed_table.hpp", 'w') as outhpp:
    all_speeds = [0.4, 0.75, 1.0, 1.1, 1.2, 1.4, 1.65, 1.80]
    all_data = float2fixedangles(all_speeds)
    # split = tuple(map(list, zip(*all_data)))
    # speedname, *fields = split
    speednameout = ",\n\t".join([f"s{x:.2f}".replace(".","_") for x in all_speeds])
    speednameout_times2 = ",\n\t".join([f"s{x * 2:.2f}".replace(".","_") for x in all_speeds])
    outhpp.write(f"""
...
""")
    # rezipped = tuple(map(list, zip(*fields)))
    for i in all_data:
      for x in i:
...
def rotate_origin_only(xy, radians):
  ...
  yy = -1 * -x * math.sin(radians) + y * math.cos(radians)
  return xx, yy

def float2fixedangles(mags):
  x_lo = []
  x_hi = []
  y_lo = []
  y_hi = []
  ...
  for mag in mags:
    for i in range(count):
      x,y = rotate_origin_only(mag, math.radians(i * (360.0/count)))
      # print(f"ang: {i * 360.0/count:.2f} mag: {mag:.2f} x,y {x:.2f},{y:.2f}")
      x = int(x * float(1 << 8))
      y = int(y * float(1 << 8))
      x_lo += [x & 0xff]
      x_hi += [(x >> 8) & 0xff]
